...
import EmptyListImg from "~/assets/svg/empty_cart.svg?react";
...
const BuildShoppingList: React.FC = () => {
  ...
  const { shoppingList, addItem, updateItem, removeItem } = shoppingListContext;

  const { editedItemKey, listID } = useParams<{
  ...
  const addItemsToList = (newItems: Item[]) => {
    newItems.forEach((item) => {
      ...
      if (!itemWithKey.category) {
        ...
        CategorizationService.getPreferredCategory(itemWithKey, shoppingList.id)
          .then((category) => {
            if (category) {
  ...
  const deleteAllItems = () => {
    shoppingList.items.forEach((item) => item.bought && removeItem(item.key));
    setShowingReally(false);
  };
  const itemList = shoppingList.items.map((item) => (
    <ItemRow
      key={item.key + item.lastUpdate}
      item={item}
      editable={editedItemKey === item.key}
      onToggleEdition={(shouldEdit) => toggleEdition(shouldEdit, item.key)}
          ...
          {itemList.length ? (
            ...
            <div>
              <span className="empty-list">
                {" "}
                Liste vide. Que voulez-vous acheter ?
              </span>
              <EmptyListImg />
            </div>
          )}
      ...
      <div className="list-actions">
        {!!shoppingList.items.filter((i) => i.bought).length && (
          <button
            ...
            type="button"
            onClick={() =>
              showingReally ? deleteAllItems() : setShowingReally(true)
            }
            onBlur={() => setShowingReally(false)}
          >
            {showingReally ? "Vraiment ?" : "Supprimer les éléments barrés"}
        ...
        <button
          ...
          type="button"
          onClick={toggleFrequentArticles}
        >
          Articles Fréquents
        </button>
      </div>
